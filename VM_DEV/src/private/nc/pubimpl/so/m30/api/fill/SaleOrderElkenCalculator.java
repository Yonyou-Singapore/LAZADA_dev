...
public class SaleOrderElkenCalculator {
	public void calculate(SaleOrderVO[] vos) {
		// 设置销售组织vid、人员、部门
		...
		// 强写的自动计算逻辑
		// 计算单位、换算率、数量、单价、金额
	    INumPriceMnyCalculator cal = new ElkenSaleOrderNPriceMnyCalculator<SaleOrderVO>(vos);
	    cal.calculate();
	}
	...
	private void setDefultInfo(IKeyValue keyValue) {
		...
		int[] sendstockrows = bodycouuitl
				.getValueNullRows(SaleOrderBVO.CSENDSTOCKORGVID);
		orgrelrule.setSendStockOrg(sendstockrows);

		int[] trafficrows = bodycouuitl
				.getValueNullRows(SaleOrderBVO.CTRAFFICORGVID);
		orgrelrule.setTrafficOrg(trafficrows);

		int[] finacerows = bodycouuitl
				.getValueNullRows(SaleOrderBVO.CSETTLEORGVID);
		...
		// 收货客户
		int[] custisnullrows = bodycouuitl
				.getValueNullRows(SaleOrderBVO.CRECEIVECUSTID);
		custrefrule.setRelaReceiveCust(custisnullrows);

		// 收货地址
		int[] eiveaddrnullrows = bodycouuitl
				.getValueNullRows(SaleOrderBVO.CRECEIVEADDRID);
		ReceiveCustDefAddrRule defaddrule = new ReceiveCustDefAddrRule(keyValue);
		defaddrule.setCustDefaultAddress(eiveaddrnullrows);

		// 国家
		int[] needchangerows = ArrayUtil
				.combinArrays(sendstockrows, finacerows);
	...
	private void setForceDefValue(IKeyValue keyValue) {
		...
		// 整单折扣
		UFDouble discountrate = keyValue
				.getHeadUFDoubleValue(SaleOrderHVO.NDISCOUNTRATE);
		...
		// 开票客户填充
		String invoicecust = keyValue
				.getHeadStringValue(SaleOrderHVO.CINVOICECUSTID);
		if (PubAppTool.isNull(invoicecust)) {
			SOCustRelaDefValueRule custrelarule = new SOCustRelaDefValueRule(
					keyValue);
		...
		if(keyValue.getHeadValue(SaleOrderHVO.DBILLDATE) == null) {
		...
		for (int i = 0; i < bodycount; i++) {
			...
			// 计划发货日期
			UFDate senddate = keyValue.getBodyUFDateValue(i,
			...
			// 要求到货日期
			UFDate receivedate = keyValue.getBodyUFDateValue(i,
			...
			// 整单折扣
			UFDouble disrate = keyValue.getBodyUFDoubleValue(i,
					SaleOrderBVO.NDISCOUNTRATE);
			if (null == disrate) {
				keyValue.setBodyValue(i, SaleOrderHVO.NDISCOUNTRATE,
						discountrate);
			}
			// 单品折扣
			UFDouble itemdisrate = keyValue.getBodyUFDoubleValue(i,
	...
	private void elkenCalculate(SaleOrderVO[] fillvos) {
